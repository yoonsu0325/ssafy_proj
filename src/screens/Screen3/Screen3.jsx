import React from "react";
import { Link } from "react-router-dom";
import "./style.css";

export const Screen3 = () => {
  return (
    <div className="screen-3" data-model-id="15:383">
      <div className="frame-80">
        <Link
          className="frame-81"
          to="/u4358u4454u4363u4469u4523u4370u4458u4358u4455u4523"
        >
          <img
            className="v-2"
            alt="V"
            src="https://c.animaapp.com/JuAZje8Q/img/--------v4-2@2x.png"
          />

          <div className="text-wrapper-104">집사</div>
        </Link>

        <div className="frame-82">
          <div className="frame-83">
            <div className="frame-84">
              <img
                className="image-9"
                alt="Image"
                src="https://c.animaapp.com/JuAZje8Q/img/image-4@2x.png"
              />

              <div className="input-2">
                <input className="container-22" placeholder="아이디" />
              </div>
            </div>

            <div className="frame-85">
              <img
                className="image-9"
                alt="Image"
                src="https://c.animaapp.com/JuAZje8Q/img/image-2@2x.png"
              />

              <div className="input-3">
                <input className="container-22" placeholder="비밀번호" />
              </div>
            </div>

            <div className="frame-85">
              <img
                className="image-9"
                alt="Image"
                src="https://c.animaapp.com/JuAZje8Q/img/image-2@2x.png"
              />

              <div className="input-3">
                <input className="container-22" placeholder="비밀번호 확인" />
              </div>
            </div>

            <div className="frame-85">
              <img
                className="image-9"
                alt="Image"
                src="https://c.animaapp.com/JuAZje8Q/img/image-3@2x.png"
              />

              <div className="input-3">
                <input className="container-22" placeholder="이메일주소" />
              </div>
            </div>

            <div className="frame-85">
              <img
                className="image-9"
                alt="Image"
                src="https://c.animaapp.com/JuAZje8Q/img/image-4@2x.png"
              />

              <div className="input-3">
                <input className="container-22" placeholder="이름" />
              </div>
            </div>

            <div className="frame-85">
              <img
                className="image-9"
                alt="Image"
                src="https://c.animaapp.com/JuAZje8Q/img/image-5@2x.png"
              />

              <div className="input-3">
                <input className="container-22" placeholder="생년월일 8자리" />
              </div>
            </div>

            <div className="frame-86">
              <img
                className="image-9"
                alt="Image"
                src="https://c.animaapp.com/JuAZje8Q/img/image-6@2x.png"
              />

              <div className="input-3">
                <input className="container-23" placeholder="휴대전화번호" />
              </div>
            </div>
          </div>

          <div className="frame-83">
            <div className="frame-84">
              <div className="text-wrapper-105">Q</div>

              <div className="input-4">
                <div className="container-24">
                  <div className="text-wrapper-106">비밀번호 찾기 질문</div>
                </div>
              </div>

              <img
                className="icon"
                alt="Icon"
                src="https://c.animaapp.com/JuAZje8Q/img/icon@2x.png"
              />
            </div>

            <div className="frame-87">
              <div className="text-wrapper-107">A</div>

              <div className="input-5">
                <input className="container-25" placeholder="답변" />
              </div>
            </div>
          </div>

          <div className="view-5">
            <div className="text-wrapper-108">회원가입</div>
          </div>
        </div>

        <div className="frame-88">
          <div className="frame-89">
            <div className="item-link">SSAFY</div>

            <div className="item-7">
              <div className="link-strong">2025.05.29</div>

              <div className="vertical-divider" />
            </div>

            <div className="item-8">
              <div className="link">WON YUN SEO</div>

              <div className="vertical-divider" />
            </div>

            <div className="item-9">
              <div className="link-2">JANG JONG WON</div>

              <div className="vertical-divider-2" />
            </div>
          </div>

          <div className="frame-90">
            <div className="text-wrapper-109">집사</div>

            <div className="text-wrapper-110">Copyright</div>

            <div className="text-wrapper-111">© BUYHOME Corp.</div>

            <div className="text-wrapper-110">All Rights Reserved.</div>
          </div>
        </div>
      </div>
    </div>
  );
};
